...
_list_tests() {
  ...
  find "test/${classlistprefix}" -name '*Test.java' | sed "s;^test/${classlistprefix}/;;g" | sort | grep -vFf .netflix/skip_tests
}
...
_run_testlist() {
    ...
    failed_tests=()
    export ANT_OPTS="$ANT_OPTS --illegal-access=permit"
...
#  sudo ant -f "${CASSANDRA_DIR}/build.xml" jar -Duse.jdk11=true
  ant build -Duse.jdk11=true
  ls "${DIST_DIR}"
...
#  ant -quiet -silent resolver-dist-lib _createVersionPropFile
  # sudo ant -quiet -silent -Duse.jdk11=true resolver-dist-lib _createVersionPropFile
  ...
  case ${target} in
    ...
    "jvm-dtest" | "jvm-dtest-novnode")
      _run_testlist "distributed" "testclasslist" "${split_chunk}" "$(_timeout_for 'test.distributed.timeout')"
      ;;
    "build_dtest_jars")
      _build_all_dtest_jars
      ;;
