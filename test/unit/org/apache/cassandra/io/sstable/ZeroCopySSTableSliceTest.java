...
public class ZeroCopySSTableSliceTest extends CQLTester
{
    ...
    public void middleRangeSliceReadsBackExactly() throws Throwable
    {
        ...
        try
        {
            ...
            assertEquals(expected.get(0), slice.first);
            assertEquals(expected.get(expected.size() - 1), slice.last);
    ...
    public void prefixSliceHasNoDeadSpace() throws Throwable
    {
        ...
        try
        {
            assertEquals(keys.get(0), materialised.reader.first);
            assertEquals(keys.get(29), materialised.reader.last);
    ...
    public void sliceToTheEndOfACompactedParentExcludesTrailingSlack() throws Throwable
    {
        ...
        try
        {
            ...
            List<DecoratedKey> keys = keysInOrder(parent);
            long dataFileLength = parent.descriptor.fileFor(Component.DATA).length();
            ...
            try
            {
                ...
                assertEquals(keys.get(keys.size() - 1), materialised.reader.last);
    ...
    public void separateRunsAreConcatenatedAndSkipWhatIsBetweenThem() throws Throwable
    {
        ...
        {
            assertEquals(expected.size(), materialised.slice.partitionCount);
            assertEquals(expected.get(0), materialised.reader.first);
            assertEquals(expected.get(expected.size() - 1), materialised.reader.last);
    ...
    public void verifierAndScrubberAcceptASliceWithAnInteriorGap() throws Throwable
    {
        ...
        try
        {
            ...
            try (Verifier verifier = new Verifier(cfs, slice, true,
    ...
    public void uncompressedSliceReadsBackExactly() throws Throwable
    {
        ...
        assertTrue("an uncompressed sstable must have a CRC.db to slice",
                   parent.descriptor.fileFor(Component.CRC).exists());
        ...
        {
            SSTableReader slice = materialised.reader;
            assertTrue(materialised.slice.components.contains(Component.CRC));
            assertFalse(materialised.slice.components.contains(Component.COMPRESSION_INFO));

            assertEquals(expected.size(), materialised.slice.partitionCount);
            assertEquals(expected.get(0), slice.first);
            assertEquals(expected.get(expected.size() - 1), slice.last);
            assertEquals(plan.dataLength, slice.uncompressedLength());
            assertEquals(plan.dataLength, slice.descriptor.fileFor(Component.DATA).length());
    ...
    public void verifierAndScrubberAcceptAnUncompressedSlice() throws Throwable
    {
        ...
        try
        {
            try (Verifier verifier = new Verifier(cfs, materialised.reader, true,
                                                 Verifier.options().extendedVerification(true).build()))
            {
                verifier.verify();
            }
    ...
    private static Materialised materialise(ColumnFamilyStore cfs, SSTableReader parent, Plan plan) throws IOException
    {
        ...
        try
        {
            try (FileChannel in = parent.descriptor.fileFor(Component.DATA).newReadChannel();
                 FileChannel out = target.fileFor(Component.DATA).newWriteChannel(File.WriteMode.OVERWRITE))
            {
            ...
            assertEquals("the receiver writes exactly what the manifest declares",
                         plan.physicalBytes, target.fileFor(Component.DATA).length());

            components.add(Component.TOC);
            SSTable.appendTOC(target, components);

            SSTableReader reader = SSTableReader.open(target, components, cfs.metadata);
                                                      ...
                                                      List<DecoratedKey> expected)
    {
        ...
        {
            RowIndexEntry entry = slice.getPosition(key, SSTableReader.Operator.EQ);
            if (wanted.contains(key))
                assertNotNull("the slice cannot find " + key, entry);
            else
                assertNull("the slice exposes " + key + ", which was not asked for", entry);
        }
    }
    ...
    {
        File data = slice.descriptor.fileFor(Component.DATA);
        try (DataIntegrityMetadata.ChecksumValidator validator = DataIntegrityMetadata.checksumValidator(slice.descriptor);
