...
public class ZeroCopySSTableSliceFuzzTest extends CQLTester
{
    ...
    public void straddlesCellBoundaries() throws Throwable
    {
        ...
        {
            DatabaseDescriptor.setFlushCompression(FlushCompression.table);
            DatabaseDescriptor.setColumnIndexSize(64);   // one row per partition, so no promoted index anyway
    ...
    private void runIteration(Case c) throws Throwable
    {
        ...
        DatabaseDescriptor.setColumnIndexSize(c.columnIndexKb);
        createTableFor(c);
        disableCompaction();
    ...
    throws Throwable
    {
        ...
        try
        {
            ...
            assertEquals("first key", expected.get(0), slice.first);
            assertEquals("last key", expected.get(expected.size() - 1), slice.last);
    ...
    {
        long physical = slice.descriptor.fileFor(Component.DATA).length();
        ...
        if (plan.compressed)
        {
            ...
            assertFalse("a compressed slice has no CRC.db", slice.descriptor.fileFor(Component.CRC).exists());
        }
        else
        {
            ...
            assertEquals("CRC.db is a header plus one checksum per cell",
                         4 + 4 * plan.cellCount(), slice.descriptor.fileFor(Component.CRC).length());
            assertFalse("an uncompressed slice has no CompressionInfo.db",
                        slice.descriptor.fileFor(Component.COMPRESSION_INFO).exists());
            ...
            try (DataIntegrityMetadata.ChecksumValidator validator =
                     DataIntegrityMetadata.checksumValidator(slice.descriptor);
                 RandomAccessReader in = RandomAccessReader.open(slice.descriptor.fileFor(Component.DATA)))
            {
                                                     ...
                                                     List<DecoratedKey> expected)
    {
        ...
        for (DecoratedKey key : all)
        {
            ...
            if (wanted.contains(key))
            {
                ...
                assertTrue("the index points outside the slice's data: " + entry.position,
                           entry.position >= 0 && entry.position < slice.uncompressedLength());
            }
            else
            {
