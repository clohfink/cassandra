...
if [ -d $CASSANDRA_HOME/build ] ; then
    jars_cnt="`ls -1 $CASSANDRA_HOME/build/nf-cassandra*.jar | grep -v 'javadoc.jar' | grep -v 'sources.jar' | wc -l | xargs echo`"
    ...
    if [ "$jars_cnt" = "1" ]; then
        cassandra_bin="`ls -1 $CASSANDRA_HOME/build/nf-cassandra*.jar | grep -v javadoc | grep -v sources`"
...
case "$JVM_VERSION" in
    ...
    21.*)
        ...
        ;;
    *)
        echo "Cassandra 4.0 requires either Java 8 (update 151 or newer) or Java 11 or Java 21."
        exit 1;
        ;;
esac
