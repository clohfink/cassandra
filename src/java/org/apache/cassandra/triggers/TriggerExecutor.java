...
public class TriggerExecutor
{
    ...
    /**
     * ThreadLocal to store ClientState for trigger execution.
     * This allows triggers to access the user's authentication context for permission checks.
     */
    private static final ThreadLocal<ClientState> clientStateThreadLocal = new ThreadLocal<>();

    /**
     * Get the ClientState for the current trigger execution, if available.
     * @return ClientState or null if not set
     */
    public static ClientState getClientState()
    {
        return clientStateThreadLocal.get();
    }
    ...
    {
        return execute(updates, null);
    }

    /**
     * Augment a partition update by executing triggers with ClientState available via ThreadLocal.
     *
     * @param updates partition update to be applied
     * @param state client state for authorization checks, may be null
     * @return the final update to be applied, the original update merged with any
     * additional mutations generated by configured triggers
     * @throws InvalidRequestException if any mutation generated by a trigger does not
     * apply to the exact same partition as the initial update
     */
    public PartitionUpdate execute(PartitionUpdate updates, ClientState state) throws InvalidRequestException
    {
        List<Mutation> intermediate = executeInternal(updates, state);
    ...
    {
        return executeInternal(update, null);
    }

    /**
     * Switch class loader before using the triggers for the column family, if
     * not loaded them with the custom class loader. Sets ClientState in ThreadLocal
     * for trigger access.
     */
    private List<Mutation> executeInternal(PartitionUpdate update, ClientState state)
    {
        ...
        ClientState previousState = clientStateThreadLocal.get();
        try
        {
            // Set ClientState in ThreadLocal for triggers to access
            if (state != null)
                clientStateThreadLocal.set(state);
        ...
        {
            // Restore previous state or clear
            if (previousState != null)
                clientStateThreadLocal.set(previousState);
            else
                clientStateThreadLocal.remove();
