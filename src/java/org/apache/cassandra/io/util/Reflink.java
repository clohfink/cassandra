 ...
 *
 * <h2>What it does</h2>
 * The ioctl makes a range of the destination point at the <em>same physical extents</em> as a range of the source
 * and bumps their reference count. Nothing is read or written, no page cache is populated, and the cost is a
 * refcount-btree update proportional to the extent count rather than the byte count. The files stay independent
 * afterwards -- either can be truncated, overwritten or unlinked, and the filesystem copies the affected blocks on
 * the first write to a shared one -- and blocks are freed only when the last reference goes away, so cloning out of
 * an sstable that is about to be unlinked hands it the extents rather than copying and then freeing them.
 * <p>
 * It is the only one of the three ways to move bytes that does not move bytes. A userspace loop copies through a
 * buffer; {@code FileChannel.transferTo} ({@code sendfile}, or {@code copy_file_range} on JDK 21+) keeps the copy in
 * the kernel but still allocates and writes every destination block -- {@code copy_file_range} <em>will</em> reflink
 * when all three arguments happen to be block aligned, but falls back to a full copy silently and with no way to
 * find out which happened. This goes through JNA instead because a caller that pads its destination specifically to
 * make sharing possible needs to know whether the padding bought anything.
 *
 * <h2>Alignment</h2>
 * The kernel requires {@code srcOffset}, {@code dstOffset} and {@code length} to be multiples of the filesystem
 * block size; unaligned arguments fail with {@code EINVAL} rather than being rounded, and the aligned interior is
 * not cloned for you. {@link #RANGE_ALIGNMENT} is a constant 64 KiB rather than a {@code statvfs} lookup, because a
 * block can never exceed the page size and 64 KiB is the largest Linux runs with -- so a multiple of 64 KiB is a
 * multiple of the block size everywhere, at a cost of at most 64 KiB of slack per range.
 *
 * <h2>Availability, and how it is discovered</h2>
 * Extent sharing needs a refcount btree: xfs formatted with {@code -m reflink=1} (the mkfs default since xfsprogs
 * 5.1), btrfs, bcachefs, OCFS2. Elsewhere the ioctl fails with {@code EOPNOTSUPP} or {@code ENOTTY}; across two
 * mounts, {@code EXDEV}. There is no cheap way to ask in advance -- {@code statfs} names the filesystem but not
 * whether reflink was enabled at mkfs time -- so support is discovered by trying and "this filesystem cannot do it"
 * is remembered per directory in {@link #unsupported}. Anything else is logged at WARN, since a failure for any
 * other reason is a bug in the caller's arithmetic.
 * <p>
 * One caveat: shared extents are cheap on disk but not in RAM, since the page cache is per inode, so bytes read
 * through both files are cached twice. That only costs when both files stay live and hot.
 */
