...
public class MmappedRegions extends SharedCloseableImpl
{
    ...
    {
        long offset = 0;
        // Where the first chunk physically starts: 0 for every sstable a writer produces, but NOT 0 for one whose
        ...
        // seeding that sum at 0 for such a file maps every segment too early: interior chunks still read correctly
        // (indexed as chunk.offset - region.offset()) but the total mapped length comes out short by the padding,
        // so the final chunk runs off the end of the last region and the file's tail is never mapped. Seeding it
        // here keeps every region's recorded offset equal to the physical offset of the first chunk it holds, the
        // invariant CompressedChunkReader.Mmap relies on.
